...
class WidgetModule: NSObject {
    ...
    func updateWorkspaceName(_ workspaceName: String?) {
        ...
        if let workspaceName = workspaceName {
            defaults.set(workspaceName, forKey: "widgetWorkspaceName")
        } else {
            defaults.removeObject(forKey: "widgetWorkspaceName")
        }
        defaults.synchronize()
        WidgetCenter.shared.reloadTimelines(ofKind: "PostHiveUpcomingWidget")
    }
    ...
    func updateLatestDeliverable(_ deliverable: [String: Any]?) {
        ...
        // Handle null/NSNull case from React Native
        guard let deliverable = deliverable, !deliverable.isEmpty else {
            ...
            defaults.removeObject(forKey: "widgetThumbnailUrl")
            defaults.synchronize()
        ...
        // Check if thumbnail URL has changed - if so, clear old cache
        if let defaults = userDefaults {
            let cachedDeliverableId = defaults.string(forKey: "widgetThumbnailDeliverableId")
            let cachedThumbnailUrl = defaults.string(forKey: "widgetThumbnailUrl")
            
            // If this is a different deliverable OR the thumbnail URL has changed, clear old cache
            if cachedDeliverableId != id || (cachedThumbnailUrl != nil && cachedThumbnailUrl != thumbnailUrl) {
                print("WidgetModule: Thumbnail URL changed or different deliverable - clearing old cache")
                defaults.removeObject(forKey: "widgetThumbnailFileName")
                defaults.removeObject(forKey: "widgetThumbnailDeliverableId")
                defaults.removeObject(forKey: "widgetThumbnailUrl")
                
                // Also delete the cached file if it exists
                if let cachedId = cachedDeliverableId, let containerURL = appGroupContainerURL {
                    let fileURL = containerURL
                        .appendingPathComponent("widget_thumbnails", isDirectory: true)
                        .appendingPathComponent("\(cachedId).jpg")
                    try? FileManager.default.removeItem(at: fileURL)
                    print("WidgetModule: Deleted old thumbnail file for deliverable \(cachedId)")
                }
            }
        }
            
        // CRITICAL: Save snapshot IMMEDIATELY so widget can render right away
        // Don't wait for thumbnail download - widget will show placeholder until image is ready
        ...
        // Trigger widget reload immediately with snapshot data
        WidgetCenter.shared.reloadTimelines(ofKind: "PostHiveDeliverableWidget")
        ...
        if let urlString = thumbnailUrl, !urlString.isEmpty, let url = URL(string: urlString) {
            ...
            downloadAndCacheThumbnail(url: url, deliverableId: id, thumbnailUrl: urlString) { [weak self] in
                // Reload widget again after thumbnail is cached
    ...
    private func downloadAndCacheThumbnail(url: URL, deliverableId: String, thumbnailUrl: String, completion: @escaping () -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: url) { [weak self] data, response, error in
            ...
            defaults.set(thumbnailUrl, forKey: "widgetThumbnailUrl")
            defaults.synchronize()

            print("WidgetModule: Cached thumbnail pointer for deliverable \(deliverableId) with URL \(thumbnailUrl)")
            
            // Reload widget after caching thumbnail
            WidgetCenter.shared.reloadTimelines(ofKind: "PostHiveDeliverableWidget")
    ...
    func updateActiveTransfer(_ transfer: [String: Any]?) {
        ...
        // Handle empty dictionary (JS "clear" case) as well as nil
        guard let transfer = transfer, !transfer.isEmpty else {
        ...
        do {
            guard let id = transfer["id"] as? String,
                ...
                print("WidgetModule: Skipping transfer update - missing required fields")
                return
            }
    ...
    func updateActivityFeed(_ activities: [[String: Any]]) {
        ...
        do {
            // Convert to ActivityItem format that matches the widget's expected structure
            ...
            if let encodingError = error as? EncodingError {
                print("WidgetModule: Encoding error details: \(encodingError)")
            }
        }
    }
    ...
    func reloadAllWidgets() {
        print("WidgetModule: Force reloading all widget timelines...")
        WidgetCenter.shared.reloadAllTimelines()
        
        // Also reload each widget kind individually to ensure they all update
        WidgetCenter.shared.reloadTimelines(ofKind: "PostHiveUpcomingWidget")
        WidgetCenter.shared.reloadTimelines(ofKind: "PostHiveDeliverableWidget")
        WidgetCenter.shared.reloadTimelines(ofKind: "PostHiveTransferWidget")
        WidgetCenter.shared.reloadTimelines(ofKind: "PostHiveActivityWidget")
        
        print("WidgetModule: All widget timelines reloaded")
    }
